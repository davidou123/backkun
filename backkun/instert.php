...
if(!$menuname){$toplo="<p align='center'>請左邊選擇一個欄位或右上角的選單來管理。</p>";}else{
...
if (mysql_num_rows($result) != 0) {
  ...
  $toplo.="<dd><span class=fixed>&nbsp; </span><img border=\"0\" src=\"img/Upload.jpg\" width=\"117\" height=\"100\"style=\"vertical-align: middle\"><BR></dd>";
for($i=0;$i<$cloummum;$i++){
	
	if($cloum[$i]=="pic")
	{	$toplo.="<dd><span class=fixed><B>".$chname[$cloum[$i]]."：</B></span><input type = \"file\" name=\"pic\" size=\"30\" >"."<br></dd>";
	}
	else if($cloum[$i]=="memo")
	{	$toplo.="<dd><span class=fixed><B>".$chname[$cloum[$i]]."：</B></span><BR><BR><textarea  name=\"".$cloum[$i]."\" cols=\"74\" rows=\"37\"></textarea>"."<br></dd>";
	}else{
	$toplo.="<dd><span class=fixed><B>".$chname[$cloum[$i]]."：</B></span> <input type=\"text\" name=\"".$cloum[$i]."\" size=\"20\" >"."<br></dd>";}
	}
...
if($_POST["name"]!=""){//新增資料----------------------
	if ($pic<>"none") {
		...
		if(file_exists("productpic/$filename")) {$rand=rand(0,100000);$filename=$rand.$filename;$_FILES[pic][name]=$rand.$filename;}
		if(!copy($pic,"productpic/$filename")) $msg.= "<font face='arial' size='2'> $name 檔案上傳失敗 ,也可能是檔案太大 請使用 back 按鍵再試一次<BR>";



$sql="INSERT INTO `$menuname` set pic='".$_FILES[pic][name]."'";
 for($i=1;$i<$cloummum;$i++){
  $sql.=",".$cloum[$i]."='".$_POST[$cloum[$i]]."' ";
 }//新增資料----------------------
 if(mysql_query($sql)){$msg.= "ok";$reurl=true;} else $msg.="寫入錯誤錯誤!，可能資料庫已經有該筆同名資料<br>";
